...
/// Starts the annual training web application.
///
/// Initializes in-memory application state, ensures the certificate output
/// directory exists, wires routes, and starts the Axum server bound to
/// `127.0.0.1:3000`.
///
/// This function is the crate-level runtime entry used by [`crate::run`] and
/// orchestrates [`home`], [`quiz_page`], [`submit_quiz`], [`result_page`], and
/// [`download_certificate`].
///
/// # Returns
///
/// Returns when server startup fails or when the serving future exits due to an
/// unrecoverable runtime/server error.
///
/// # Errors
///
/// This function logs operational failures (directory creation, browser launch,
/// bind failures, and serve failures) to stderr rather than returning them.
///
/// # Examples
///
/// ```no_run
/// # use annual_training::run;
/// # #[tokio::main]
/// # async fn main() {
/// run().await;
/// # }
...
///
/// The page includes module metadata seeded by [`seed_videos`] and provides the
/// primary navigation path to [`quiz_page`].
///
/// # Returns
///
...
///
/// Questions are selected by [`QuizService`] via [`QuizOperations::choose_questions`],
/// then serialized into hidden identifiers consumed by [`submit_quiz`].
///
/// # Returns
///
/// HTML markup for the quiz form.
...
/// # Returns
///
/// A redirect to `/quiz` for invalid/missing question identifiers, or a redirect
/// to `/result?ticket=...` for processed submissions.
///
/// # Errors
///
/// Certificate persistence failures are logged and do not fail the request.
...
///
/// Uses the `ticket` query parameter generated by [`submit_quiz`] to load
/// score/pass data and optional certificate download links.
///
/// # Returns
///
/// HTML containing either a missing-ticket/unknown-ticket message or a complete
/// exam result view.
...
/// # See also
///
/// - [`CertificateService::write_certificate_files`]
...
///
/// The returned data set is rendered by [`home`].
///
/// # Returns
///
/// Static module metadata used for the landing page video cards.
