...
pub(crate) trait QuizOperations {
    ...
    /// # Parameters
    ///
    /// - `raw_answers`: Untrusted form values from [`crate::app::submit_quiz`].
    ///
    /// # Returns
    ///
    /// A map containing only parseable question-answer pairs.
    ...
    /// # Returns
    ///
    /// A [`QuizEvaluation`] with score, total questions, and pass/fail outcome.
...
impl QuizOperations for QuizService {
    ...
    /// # Errors
    ///
    /// Invalid numeric values are ignored rather than treated as fatal errors.
    fn parse_answers(&self, raw_answers: &HashMap<String, String>) -> HashMap<String, usize> {
        raw_answers
            .iter()
    ...
    /// # Parameters
    ///
    /// - `raw_ids`: Comma-separated question IDs generated by
    ///   [`QuizOperations::choose_questions`].
    ///
    /// # Returns
    ///
    /// Only matching questions; unknown IDs are ignored.
    fn select_questions_by_id(&self, question_bank: &[Question], raw_ids: &str) -> Vec<Question> {
        raw_ids
            .split(',')
    ...
    /// # Returns
    ///
    /// A [`QuizEvaluation`] where `passed` is true when score percentage is at
...
/// # Returns
///
/// A stable list of [`Question`] values used by [`QuizService`] and rendered by
/// [`crate::app::quiz_page`].
